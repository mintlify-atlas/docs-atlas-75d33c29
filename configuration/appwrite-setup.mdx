---
title: "Configuring Appwrite for AlejoTaller"
...
<Tip>
Enable **Realtime** on the `sales` collection (Appwrite console → collection → Settings → Realtime). The operator Android app (`alejotallerscan`) subscribes to Appwrite Realtime to detect when `buy_state` changes so it can trigger the publisher microservice and push the confirmation to Pusher.
</Tip>
