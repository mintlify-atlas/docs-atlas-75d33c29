---
title: "End-to-End Sale Lifecycle in AlejoTaller"
sidebarTitle: "Sale Flow"
description: "Trace a complete sale from customer order creation through operator verification to real-time confirmation, covering Appwrite persistence, Pusher events, and local state reconciliation."
---
...
flowchart TD
    ...
    C --> D[Save to Room / Dexie locally]
    D --> E[Push to Appwrite]
    E --> F[Send Telegram notification]
    F --> G[Operator app loads pending sales from Appwrite]
...
<Steps>
  ...
  <Step title="Sale persisted locally then pushed remotely">
    `SaleOfflineFirstRepository.save()` writes to Room first, then attempts the remote push with up to three retries. A network failure at this point leaves the sale in `UNVERIFIED` state in the local database; it will be pushed on the next `SyncSalesCaseUse` invocation.
  </Step>

  <Step title="Telegram notification sent">
    A Telegram message is dispatched via `TelegramNotificatorImpl` before the repository call. This gives the business owner immediate visibility of new orders through a channel that does not require the operator app to be open.
  </Step>
</Steps>
