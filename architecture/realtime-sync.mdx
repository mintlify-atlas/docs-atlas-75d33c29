...
description: "How AlejoTaller uses Pusher Channels for customer sale-verification events and Appwrite Realtime for operator sync across two isolated reactive layers."
---
...
```mermaid
sequenceDiagram
    participant Op as Operator App (AlejoTallerScan)
    ...
    Op->>Pub: POST /sale-verification/publish
    Note over Op,Pub: body: saleId, userId, decision
    Pub->>Pub: validateCommand()
    Pub->>PS: pusher.trigger(sale-verification-{userId}, sale:confirmed / sale:rejected, payload)
